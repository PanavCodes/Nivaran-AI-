...
| `app/routers/complaint_routes.py` | ✅ | Complaint submission with AI intake analysis |
| `app/routers/cluster_routes.py` | ✅ | Active clusters, nearby lookup, detail, assign, resolve, audit |
| `app/routers/admin_routes.py` | ✅ | Analytics KPI endpoint and WebSocket room management |
...
| Radar Intake | `/report` | ✅ | Drag-and-drop photo upload with AI pre-fill, nearby cluster sidebar, submission feedback modal |
| Mission Control | `/admin` | ✅ | Full-screen split view: Leaflet map with pulsing pins + cluster detail flyout + KPI analytics row |
| Task Force Terminal | `/technician` | ✅ | Mobile-first SLA queue with live countdown timers, dual-proof photo verification |
...
└── frontend/
    ├── src/
    │   ├── app/             # Next.js pages (landing, login, register, report, admin, technician)
    │   ├── components/      # UI primitives + NivaranMap
...
- Python 3.11+
- Node.js 18+
- PostgreSQL 14+ with pgvector extension (or use the `ankane/pgvector` Docker image)
...
docker run -d --name nivaran_db \
  ...
  -p 5432:5432 \
  -v ./backend/db/schema.sql:/docker-entrypoint-initdb.d/schema.sql \
...
| POST | `/api/v1/complaints/analyze` | Analyze image without submission |
