...
export const IncidentMemoModal: React.FC<IncidentMemoModalProps> = ({
  ...
  const refCode = `UNIV/ENG-EST/2026/FL-${cluster.floor}/${cluster.id.slice(0, 8).toUpperCase()}`;

  const memoText = `OFFICE OF THE PRINCIPAL & REGISTRAR — CAMPUS WORKS DIVISION
Ref: ${refCode}
Date: ${dateStr}
Subject: Urgent Engineering & Maintenance Redressal Directive — Floor ${cluster.floor}
To: Executive Engineer & Estate Works Officer
Copy to: Chief Warden (Hostel & Campus Facilities), Accounts Section (Contractor Bill Clearance)
From: Nivaran Operations & Campus Audit Division
...
3. AUDIT & BILL CLEARANCE DIRECTIVE:
Contractor and maintenance crew must submit photographic before/after proof via Nivaran portal. Accounts section will not clear vendor invoices without verified timestamped system audit.

Approved by: ________________________ (Principal / Registrar)
`;
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                University Directive & Incident Memo
                <Badge variant="emergency" className="text-[10px] py-0">Statutory</Badge>
              </h3>
              <p className="text-xs text-slate-500">Official campus facility memorandum for Estate & Accounts record</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-slate-300/80 bg-[#fdfdfd] p-6 font-mono text-xs text-slate-800 leading-relaxed shadow-sm space-y-3">
          <div className="text-center border-b border-slate-200 pb-3">
            <div className="flex items-center justify-center gap-2 text-slate-900 font-bold text-sm tracking-wide font-sans">
              OFFICE OF THE PRINCIPAL & REGISTRAR · CAMPUS WORKS DIVISION
            </div>
            <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">
              Estate Management, Maintenance & Grievance Redressal Section
            </p>
          </div>
          ...
          <div className="pt-2">
            <p className="font-bold text-slate-900">SUBJECT: Urgent Redressal Directive — Floor {cluster.floor}</p>
            <p className="text-slate-500 text-xs mt-0.5">TO: Executive Engineer & Estate Officer | COPY: Accounts Section (Contractor Clearance)</p>
          </div>
